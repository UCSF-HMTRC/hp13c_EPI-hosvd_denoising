...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp('xyz_tool: Create_New_Button');
fig = gcf;
...
if ~isempty(hToolbar) & isempty(findobj(hToolbar, 'Tag', 'figxyz_tool'))
   ...
   f=[f,    15    29    43    57    71    85    99   113   127   141   155   169   183   197 211];
   
   button_image(f) = 0;
   button_image = repmat(button_image, [1,1,3]);
   
   buttontags = {'figWindowLevel', 'figPanZoom', 'figROITool', 'figViewImages', 'figxyz_tool', 'figRotateTool', 'figProfileTool'};
   separator = 'off';
   ...
   set(hNewButton, 'Cdata', button_image, ...
      'OnCallback', 'xyz_tool(''Activate_Point_Tool'')',...
      'OffCallback', 'xyz_tool(''Deactivate_Point_Tool'')',...
      'Tag', 'figxyz_tool', ...
      'TooltipString', 'XYZ Measurement Tool',...
	  'Separator', separator, ...
      'UserData', [], ...
      'Enable', 'on');   
end;
...
if ~isempty(hToolMenu) & isempty(findobj(hToolMenu, 'Tag', 'menuxyz_tool'))
  ...
  hPointToolMenu   = findobj(hToolMenu, 'Tag', 'menuxyz_tool');
  ...
  set(hNewMenu, 'Tag', 'menuxyz_tool','Label',...
      'Point Measurements',...
      'CallBack', 'xyz_tool(''Menu_Point_Tool'')',...
      'Separator', separator,...
      'UserData', hNewButton...
  ); 
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp('xyz_tool:Activate_Point_Tool');

if nargin ==0
    set(0, 'ShowHiddenHandles', 'On');
    hNewButton = gcbo;
    set(findobj('Tag', 'menuxyz_tool'),'checked', 'on');
else
...
if ~isempty(hToolbar)
	...
	PM = findobj(hToolbar_Children, 'Tag', 'figxyz_tool');
...
close_str = [ 'hNewButton = findobj(''Tag'', ''figxyz_tool'');' ...
        ' if strcmp(get(hNewButton, ''Type''), ''uitoggletool''),'....
        ' set(hNewButton, ''State'', ''off'' );' ...
        ' else,  ' ...
        ' xyz_tool(''Deactivate_Point_Tool'',hNewButton);'...
...
%set(h_all_axes, 'ButtonDownFcn', 'xyz_tool(''Set_Current_Axes'', gca)');
set(fig, 'CurrentAxes', h_axes);

handlesPM.Axes = h_all_axes;
%for i = 1:length(h_all_axes)
%	set(findobj(h_all_axes(i), 'Type', 'image'), 'ButtonDownFcn', 'xyz_tool(''Measure'', gca)'); 	
%end;
set(fig, 'WindowButtonDownFcn',  ['xyz_tool(''Measure_Start'',',num2str(fig),')']);
...
set(fig, 'Closerequestfcn', [ old_CRF , ',xyz_tool(''Close_Parent_Figure'')']);
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp('xyz_tool:Deactivate_Point_Tool');

if nargin ==0
    set(0, 'ShowHiddenHandles', 'On');    
    hNewButton = gcbo;
    set(findobj('Tag', 'menuxyz_tool'),'checked', 'Off');
else
    hNewButton = varargin{1};
end;
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp('xyz_tool: Measure_Start');
set(fig,'WindowButtonMotionFcn',['xyz_tool(''Get_Coordinates'')']);
set(fig,'WindowButtonDownFcn'  ,['xyz_tool(''Measure_End'',',num2str(fig),')']);
...
% added by PK
	...
	xyz=getappdata(currentaxes(1),'xyz_data');
    XYZ=squeeze(xyz(currentpoint(1,1),currentpoint(1,2),:));
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp('xyz_tool: Set_Current_Axes');
