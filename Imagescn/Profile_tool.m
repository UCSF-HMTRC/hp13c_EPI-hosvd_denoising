...
% update_list
% DH
mmode=get(handlesTool.mmode_checkbox,'value');


if mmode==1 && nargin~=0
    % deal with unlinked profiles that still need updating for mmode figure
    curr_profile = update_list(1,1);
    update_list = [];
    for j = 1:size(handlesTool.Profile_info_table,2)
        % for each axes - update profile if it exists
        if ~isempty(handlesTool.Profile_info_table(curr_profile,j).Profile_Exists)
%             update_list
            update_list(end+1,:) = [curr_profile,j];
		end;    
    end
end
% update_list
...
for i = 1:size(update_list,1)
	if    handlesTool.Profile_info_table(update_list(i,1),update_list(i,2)).Profile_Type ~= 4
        ...
        improfile_method='bilinear'; % or 'bilinear', 'bicubic', or 'nearest'
		[Px,Py, P] = improfile(im,xpts,ypts,Profile_points, improfile_method);
		dist_interp = [ 0 cumsum(sqrt( (diff(Px - Px(1)).^2 + diff(Py - Py(1)).^2)))' ];
		
        if mmode==1 % PK*
            %im0= getappdata(handlesTool.Current_Axes, 'ImageData'); %PKver
            im0= getappdata(handlesTool.h_all_axes(update_list(i,2)), 'ImageData');  %DHver - get right im data

            %             I = get(findobj(get(h_line, 'Parent'), 'Type', 'Image'), 'CData');
            Profile_points_mmode=256;
            for im_no=1:size(im0,3)
                [Px0, Py0, P0(:,im_no,i)] = improfile(im0(:,:,im_no),xpts,ypts,Profile_points_mmode, improfile_method);
            end
            
            if 0% THICK PROFILE
                % determine vector normal to profile line
                profile_vector=[xpts(end)-xpts(1), ypts(end)-ypts(1), 0];
                normal_vector = cross(profile_vector,[0 0 1]);
                normal_vector = normal_vector(1:2)/norm(normal_vector(1:2));
                for offset=[-1:1]
                for im_no=1:size(im0,3)
                    [Px0, Py0, P0(:,im_no,i,offset+2)] = improfile(im0(:,:,im_no),xpts+offset*normal_vector(1),ypts+offset*normal_vector(2),Profile_points_mmode, improfile_method);
                end
                end
                P0=mean(P0,4);
            end
    ...
    %PK*
    if mmode==1 & ~isempty(P0)% PK*
        f_old  = findobj(0, 'tag','mmode');
        if ~isempty(f_old)
            mmode_position= get(f_old(1),'position');
            close(f_old)
        end
        f_new=figure('tag','mmode','visible','off');
%         aspect_ratio=Profile_points_mmode/size(P0,2);
%         Nrepetitions=ceil(aspect_ratio); % repeat in time to make displayed image approx square 
        Nrepetitions=3; % fixed number of heartbeats to be repeat on time (horizontal) axis
        Immode = repmat(P0,[1 Nrepetitions 1]);
        Immode=interpimages2(Immode,'linear',[Profile_points_mmode Profile_points_mmode]);
        imagescn(Immode)  % DHver
        %imagescn(repmat(P0,[1 12])) %PKver
        if exist('mmode_position','var')
            curr_position = get(f_new, 'position');  % DH
            set(f_new,'position',[mmode_position(1:2), curr_position(3:4)]); %DH (maintain size)
            %set(f_new,'position',mmode_position); % PKver            
        end
    end
...
if  hT.Profile_info_table(Current_Profile(1), Current_Profile(2)).Profile_Type ~= 4
	...
	Profile_points = max(size(im))*3;
    improfile_method='bilinear';
	[Px, Py, Y] = improfile(im,xpts,ypts,Profile_points, improfile_method);
	if get(hT.handlesPlot.FFT_checkbox, 'Value')
		Y = fft_convert(Y);
	end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function Create_Profile_Image_Figure;
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function to create and initialize the Profile Plotfigure
%global DB; if DB disp(['Profile_Tool: ', Get_Current_Function]); end;
...
fig_image = openfig('Profile_image_figure');
set(fig_image, 'Name', 'Profile Image' ,...
    'Tag', 'Prof_I_figure' ,...
    'Resize' , 'On',...
    'CloseRequestfcn', close_str...
    );

handlesPlot = guihandles(fig_images);

set(fig_image, 'Renderer', 'zbuffer');
set(handlesPlot.Single_Profile_Plot_axes, 'Drawmode', 'Fast', 'Userdata', {'auto', 'auto'});
...
handlesTool = guidata(findobj('Tag', 'Prof_I_figure'));
handlesTool.handlesPlot = handlesPlot;
handlesTool.Plot_Image_figure = fig_image;
handlesTool = Create_Multiple_Axes(handlesTool);
multiple_plot_axes =handlesTool.handlesPlot.h_plot_axes';
