...
elseif strcmp(cmap2,'Less...')
	...
	if strcmp(cmap2,'Vga') % vga does takes only 16 colors...
		eval(['set(', num2str(userdata{1}) , ', ''colormap'' , colormap(' ,lower(cmap2),'))' ]);        
	else
		eval(['set(', num2str(userdata{1}, '%1.15g') , ', ''colormap'' , colormap(' ,lower(cmap2), '(' num2str(size(userdata{2},1)), ')))' ]);
	end;
...
hNewMenu = gcbo;
% checked=  umtoggle(hNewMenu);%% PK* 20071004
checked = get(hNewMenu,'checked');
